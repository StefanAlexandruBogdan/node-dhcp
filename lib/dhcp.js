...
Server.prototype = {
  ...
  _getOptions: function (pre, required, requested, originalReq) {
    ...
    for (var req of required) {
      ...
      if (Options.opts[req] !== undefined) {
        ...
        if (pre[req] === undefined) {
          pre[req] = this.config(Options.opts[req].config, originalReq);
        }
    ...
    if (requested) {
      ...
      for (var req of requested) {
        ...
        if (Options.opts[req] !== undefined) {
          ...
          // Take the first config value always
          if (pre[req] === undefined) {
            var val = this.config(Options.opts[req].config, originalReq);
          ...
          this.emit('error', 'Unknown option ' + req, originalReq);
        }
      }
    }
    ...
    if (forceOptions instanceof Array) {
      for (let option of forceOptions) {
        ...
        if (id !== undefined && pre[id] === undefined) {
          pre[id] = this.config(option, originalReq);
        }
      }
    }
    return pre;
  ...
  _selectAddress: function (clientMAC, req) {
    ...
    var _static = this.config('static', req);
    ...
    var randIP = this.config('randomIP', req);
    var _tmp = this.config('range', req);
    ...
    var ips = [this.config('server', req)]; // Exclude our own server IP from pool
    var oldestMac = null;
  ...
  sendOffer: function (req) {
    ...
    if (req.options[97]) {
      if (req.options[55].indexOf(97) === -1) {
    ...
    var ans = {
      ...
      siaddr: this.config('server', req), // next server in bootstrap. That's us
      giaddr: req.giaddr,
    ...
    this._send(this.config('broadcast', req), ans);
  ...
  sendAck: function (req) {
    ...
    var ans = {
      ...
      ciaddr: req.ciaddr,
      yiaddr: this._selectAddress(req.chaddr), // my offer
      siaddr: this.config('server', req), // server ip, that's us
    ...
    this.emit('bound', this._state, ans);
...
Client.prototype = {
  ...
  sendRelease: function (req) {
    ...
    var ans = {
      ...
      secs: 0, // 0 or seconds since DHCP process started
      flags: 0,
      ciaddr: this.config('server', req),
      yiaddr: INADDR_ANY,
      siaddr: INADDR_ANY,
      giaddr: INADDR_ANY,
      chaddr: this.config('mac', req),
      sname: '', // unused
      file: '', // unused
      options: {
        53: DHCPRELEASE,
